...
export class Game {
    ...
    private setupEventListeners(): void {
        ...
        if (startBattleBtn) startBattleBtn.onclick = () => this.startBattle();
        if (stopBattleBtn) stopBattleBtn.onclick = () => this.stopBattle();
        if (nextStageBtn) nextStageBtn.onclick = () => this.nextStage();
        if (prevStageBtn) prevStageBtn.onclick = () => this.prevStage();
        if (levelUpBtn) levelUpBtn.onclick = () => this.showLevelUpDialog();
        ...
        if (upgradeAlchemyBtn) upgradeAlchemyBtn.onclick = () => this.upgradeAlchemy();
        if (collectAlchemyBtn) collectAlchemyBtn.onclick = () => this.collectAlchemy();
    ...
    private updateUI(): void {
        ...
        if (stageProgress) {
            this.uiManager.updateStageInfo(
                ...
                stageProgress.enemiesDefeated,
                stageProgress.totalEnemies
            );
        }
    }
    ...
    private updateStageButtons(): void {
        const currentStage = this.battleManager.getStageProgress()?.currentStage || 1;
        const prevBtn = document.getElementById('prev-stage-btn') as HTMLButtonElement;
    ...
    private handleBattleResult(result: BattleResult): void {
        ...
        if (isPlayerWin) {
            this.uiManager.addBattleMessage(
                `胜利！造成 ${result.totalDamageDealt || 0} 伤害，获得奖励！`, 
                'heal'
            );
        } else {
    ...
    private handleEnemyDefeated(enemy: any): void {
        this.uiManager.addBattleMessage(
            `${enemy.name} 被击败！获得 ${enemy.rewards.experience} 经验，${enemy.rewards.gold} 金币`, 
            'heal'
        );
    ...
    private handlePlayerLevelUp(newLevel: number): void {
    ...
    private handlePlayerRevive(event: ReviveEvent): void {
    ...
    private showLevelUpDialog(): void {
        if (this.player.availablePoints <= 0) return;
        
        this.uiManager.showModal(
            '属性升级',
            `可用属性点: ${this.player.availablePoints}\n选择要提升的属性:`,
            [
    ...
    private allocateAttribute(attribute: keyof typeof this.player.attributes, points: number): void {
    ...
    upgradeAttribute(attribute: string): void {
        ...
        if (this.player.allocateAttribute(attribute as keyof typeof this.player.attributes, 1)) {
    ...
    private showOfflineRewards(offlineTime: number, saveData: any): void {
        // 创建临时玩家对象计算离线收益
